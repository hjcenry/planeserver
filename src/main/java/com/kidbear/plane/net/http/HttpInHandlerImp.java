package com.kidbear.plane.net.http;
...
import com.kidbear.plane.util.encrypt.XXTeaCoder;
...
public class HttpInHandlerImp {
	...
	private void getHandle(final ChannelHandlerContext ctx,
		...
		if (typeList.get(0).equals("hotfix")) {// 热更新
			List<String> channelList = params.get("channel");
			List<String> baseList = params.get("base");
			HotFixMgr.getInstance().queryVersion(ctx, channelList.get(0),
					baseList.get(0));
		} else {
